    ...
    public static function loadClass(ReflectionClass $refl, Reader $reader)
    {
        ...
        foreach ($methods as $method) {
            ...
            for ($i = 0; $i < count($routes); ++$i) {
                $router->addRoute(new Route (['path' => $routes[$i]->path, 'classMethod' => $method->name, 'class' => $refl->name, 'httpMethod' => ($httpMethod) ? $httpMethod->allow : null, 'secure' => ($secure) ? $secure->roles : null, 'feature' => ($feature) ? $feature->features : null]));
            }
        }
    }
}
