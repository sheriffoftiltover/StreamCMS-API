...
class TwitterAuthHandler
{
    ...
    public function getAuthenticationUrl()
    {
        ...
        $tmhOAuth = new tmhOAuth (['consumer_key' => $authConf ['clientId'], 'consumer_secret' => $authConf ['clientSecret'], 'token' => $authConf ['token'], 'secret' => $authConf ['secret'], 'curl_connecttimeout' => Config::$a ['curl'] ['connecttimeout'], 'curl_timeout' => Config::$a ['curl'] ['timeout'], 'curl_ssl_verifypeer' => Config::$a ['curl'] ['verifypeer']]);
        $code = $tmhOAuth->apponly_request(['without_bearer' => true, 'method' => 'POST', 'url' => $tmhOAuth->url('oauth/request_token', ''), 'params' => ['oauth_callback' => $callback]]);
        if ($code != 200) {
    ...
    public function authenticate(array $params)
    {
        ...
        $tmhOAuth = new tmhOAuth (['consumer_key' => $twitterOAuthConf ['clientId'], 'consumer_secret' => $twitterOAuthConf ['clientSecret'], 'token' => $oauth ['oauth_token'], 'secret' => $oauth ['oauth_token_secret'], 'curl_connecttimeout' => Config::$a ['curl'] ['connecttimeout'], 'curl_timeout' => Config::$a ['curl'] ['timeout'], 'curl_ssl_verifypeer' => Config::$a ['curl'] ['verifypeer']]);
        $code = $tmhOAuth->user_request(['method' => 'POST', 'url' => $tmhOAuth->url('oauth/access_token', ''), 'params' => ['oauth_verifier' => trim($params ['oauth_verifier'])]]);
        if ($code != 200) {
