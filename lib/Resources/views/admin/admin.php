...
<section class="container">
    ...
    <div class="content content-dark clearfix">
        ...
        <div id="userlist" data-size="<?= Tpl::out($model->size) ?>" data-page="<?= Tpl::out($model->page) ?>"
             class="stream stream-grid" style="width:100%;">
                    ...
                    <form class="form-inline pull-left" role="form">
                        <ul class="pagination" style="margin: 0 15px 0 0;">
                            ...
                            <?php for ($i = max(1, $model->users['page'] - 2); $i <= min($model->users['page'] + 2, $model->users['totalpages']); $i++): ?>
                                <li <?= ($model->users['page'] == $i) ? 'class="active"' : '' ?>><a
                                            data-page="<?= $i ?>" href="?page=<?= $i ?>"><?= $i ?></a></li>
                            <?php endfor; ?>
                            <li><a data-page="<?= $model->users['totalpages'] ?>"
                                   href="?page=<?= $model->users['totalpages'] ?>">End</a></li>
                        </ul>
                    </form>
                <?php endif; ?>
